...
public class KeyguardSliceProviderPixeldust extends KeyguardSliceProvider implements SmartSpaceUpdateListener {
    ...
    private static class AddShadowTask extends AsyncTask<Bitmap, Void, Bitmap> {
        ...
        private final WeakReference<KeyguardSliceProviderPixeldust> mProviderReference;
        private final SmartSpaceCard mWeatherCard;

        AddShadowTask(KeyguardSliceProviderPixeldust keyguardSliceProviderPixeldust, SmartSpaceCard smartSpaceCard) {
            mProviderReference = new WeakReference<>(keyguardSliceProviderPixeldust);
            mWeatherCard = smartSpaceCard;
            mBlurRadius = keyguardSliceProviderPixeldust.getContext().getResources().getDimension(R.dimen.smartspace_icon_shadow);
        }
        ...
        public void onPostExecute(Bitmap bitmap) {
            KeyguardSliceProviderPixeldust keyguardSliceProviderPixeldust;
            synchronized (this) {
                mWeatherCard.setIcon(bitmap);
                keyguardSliceProviderPixeldust = (KeyguardSliceProviderPixeldust) mProviderReference.get();
            }
            if (keyguardSliceProviderPixeldust != null) {
                keyguardSliceProviderPixeldust.notifyChange();
            }
        }
    ...
    public Slice onBindSlice(Uri uri) {
        Trace.beginSection("KeyguardSliceProviderPixeldust#onBindSlice");
        Slice slice;
